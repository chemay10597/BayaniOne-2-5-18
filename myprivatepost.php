<?php
  ...
  $result = mysqli_query($connect,"SELECT * FROM posts INNER JOIN donation_campaign ON post.post_id=donation_campaign.post_id INNER JOIN users ON users.user_id=post.user_id WHERE post_type='donation campaign' AND post_status='timeline' AND username='". $_SESSION["username"] ."'");
